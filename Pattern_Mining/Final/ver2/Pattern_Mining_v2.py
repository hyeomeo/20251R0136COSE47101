...
df = pd.read_csv("travel_data_final_v2.csv")
...
dataset = df_filt.apply(
    lambda r: [f"CLUSTER_{r['CLUSTER_V2']}", r['VISIT_AREA_NM']], axis=1
).tolist()
...
rules_final[['antecedents', 'consequents', 'support', 'confidence']].to_csv("visitarea_cluster_rules_v2.csv",index=False,encoding="utf-8-sig")   
