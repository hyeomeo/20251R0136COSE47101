...
df = pd.read_csv("preprocessed_travel_data_final.csv")
...
area_keep = area_keep[area_keep >= 30].index            # 30회 이상 지역만
df_filt   = df[df['VISIT_AREA_NM'].isin(area_keep)].copy()
dataset = df_filt.apply(
    lambda r: [r['VISIT_AREA_NM'], f"CLUSTER_{r['CLUSTER']}"],
    axis=1
).tolist()
...
frequent_itemsets = fpgrowth(df_tx, min_support=0.008, use_colnames=True)
...
plt.title('VISIT_AREA_NM <-> CLUSTER  (지역 ≥ 30회, FP-Growth)', fontsize=14)
plt.axis('off');
plt.tight_layout();
plt.show()
